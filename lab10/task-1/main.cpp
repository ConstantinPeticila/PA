...
class Task {
 ...
 private:
	...
	static bool comparator(const tuple <int,int,int> &lh ,const tuple <int,int, int> &rh )
	{
		return get<0>(lh) < get<0> (rh);
	}
	void read_input() {
		ifstream fin("in");
		fin >> n >> m;
	...
	int get_result() {
		...
		for(int i = 0; i < edges.size() ; i++) {
			if(find_root(get<1>(edges[i])) != find_root(get<2>(edges[i]))) {
				wg += get<0>(edges[i]);
				merge_forests(find_root(get<1>(edges[i])),find_root(get<2>(edges[i])));
			}
		}
